...
async function initPage() {
    try {
        ...
        // Populate transaction modal category select
...
async function exportTransactions() {
    try {
        ...
        transactions.forEach(trans => {
            const category = categories.find(c => c.id === trans.category_id);
            csv += `${trans.transaction_date},"${category?.name || 'Unknown'}",${trans.amount},${trans.transaction_type},"${trans.payment_method || ''}","${(trans.notes || '').replace(/"/g, '""')}"\n`;
        });
